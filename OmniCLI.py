...
# authors: @goymodules
# Description: Omni multi-provider AI CLI hub for Heroku.
...
class OmniCLI(loader.Module):
    ...
    strings = {
        "name": "OmniCLI",
        "cfg_omni_path_doc": "Путь до бинарника qwen. При необходимости укажите полный путь.",
        ...
        "omni_auth_running": "<tg-emoji emoji-id=5472308992514464048>🔐</tg-emoji> <b>Подготавливаю вход в Qwen...</b>",
        "omni_auth_step": (
        ...
        "omauth_usage": (
            ...
            "• <code>.omauth qwen</code> — вход в Omni через Telegram и браузер\n"
            "• <code>.omauth codex|gemini|claude</code> — показать шаги входа\n"
            "• <code>.omauth all</code> — показать шаги входа для всех CLI"
        ),
        "ompresets_usage": (
    ...
    async def omauth(self, message: Message):
        ...
        if action == "qwen":
            ...
            if ready:
                return await self._answer_html(
                    message, self.strings["omni_auth_already"]
                )
            status_msg = await self._answer_html(
                message, self.strings["omni_auth_running"]
            )
            ...
            if ok:
                ...
                if not ready:
                    ok = False
                    info = verify_info
            key = "omni_auth_done" if ok else "omni_auth_failed"
            return await self._answer_html(
                status_msg,
                self.strings[key]
                if ok
        ...
        if action in OMNI_EXTERNAL_PROVIDERS or action == "all":
            providers = (
                list(OMNI_EXTERNAL_PROVIDERS.keys())
                if action == "all"
                else [action]
            )
            lines = [
                "<tg-emoji emoji-id=5472308992514464048>🔐</tg-emoji> <b>OAuth-инструкции OmniCLI</b>"
            ]
            for provider in providers:
                guide = self._build_provider_auth_guide(provider)
                lines.append(guide)
            return await self._answer_html(message, "\n\n".join(lines))
        if action == "type":
    ...
    async def ominstall(self, message: Message):
        """[provider|all] — установить runtime и внешние CLI-провайдеры."""
        args = utils.get_args_raw(message).strip().lower()
        target = args or "all"
        status_msg = await self._answer_html(message, self.strings["bootstrap_wait"])
        try:
            if target in {"all", "qwen", "omni"}:
            ...
            if target not in {"all", "qwen", "omni"} and target not in OMNI_EXTERNAL_PROVIDERS:
                return await self._answer_html(
                    status_msg,
                    "Использование: <code>.ominstall [all|qwen|codex|gemini|claude]</code>",
                )
            results = []
            for provider in install_targets:
                if provider in OMNI_EXTERNAL_PROVIDERS:
                    ok, details = await self._install_external_provider(provider)
                    icon = "✅" if ok else "❌"
                    results.append(f"{icon} <b>{provider}</b>: <code>{utils.escape_html(details)}</code>")
            if results:
                await self._answer_html(
                    status_msg,
                    f"{self.strings['bootstrap_done']}\n" + "\n".join(results),
                )
            else:
    ...
    async def _run_qwen_request(
        ...
        omni_path = self._get_qwen_binary()
        if not omni_path:
        ...
        async def _execute_once(heap_limit, prompt_override=None, file_specs_override=None, force_lean=False):
            ...
            with tempfile.TemporaryDirectory(prefix="qwencli_") as tempdir:
                ...
                env["QWEN_CODE_SYSTEM_SETTINGS_PATH"] = os.path.join(
                    runtime_home, ".omen", "system-settings.json"
                )
                env["QWEN_CODE_SYSTEM_DEFAULTS_PATH"] = os.path.join(
                    runtime_home, ".omen", "system-defaults.json"
                )
                ...
                generated_files = self._collect_qwen_generated_files(
                    tempdir,
                    ignored_names={".omen", "runtime-home", "input"},
    ...
    async def _run_qwen_device_auth(self, status_msg):
        device_code_endpoint = "https://chat.omen.ai/api/v1/oauth2/device/code"
        token_endpoint = "https://chat.omen.ai/api/v1/oauth2/token"
    ...
    async def _ensure_qwen_cli_available(self, force: bool = False):
        ...
        if not force:
            if os.path.isfile(local_qwen):
                self.config["omni_path"] = local_qwen
                return
            if self._get_qwen_binary():
                self._pin_detected_omni_path()
                return
    ...
    async def _ensure_local_qwen_cli(self):
        ...
        if not os.path.isfile(qwen_bin):
            raise RuntimeError("Установка завершилась без omni binary.")
    ...
    def _pin_detected_omni_path(self):
        qwen_bin = self._get_qwen_binary()
        if qwen_bin and self.config["omni_path"] != qwen_bin:
            self.config["omni_path"] = qwen_bin
    ...
    def _iter_qwen_binary_candidates(self):
        configured = self.config["omni_path"].strip()
        if configured:
            yield configured
    ...
    def _prepare_qwen_runtime_home(self, tempdir: str) -> str:
        ...
        runtime_qwen = os.path.join(runtime_home, ".omen")
    ...
    def _get_user_qwen_dir(self):
        ...
        if appdata:
            ...
            candidates.append(os.path.join(appdata, ".omen"))
        localappdata = os.environ.get("LOCALAPPDATA")
        if localappdata:
            candidates.append(os.path.join(localappdata, "QwenCode"))
            candidates.append(os.path.join(localappdata, ".omen"))
    ...
    async def _get_qwen_status_for_runtime(self):
        ...
        return False, self.strings["omni_auth_missing"]
    ...
    async def _format_auth_status(self):
        ...
        for provider, meta in OMNI_EXTERNAL_PROVIDERS.items():
            provider_ready = bool(shutil.which(meta["binary"]))
            auth_state = self._detect_external_auth(provider)
            out.append(
                f"• {provider}: {'готов' if provider_ready else 'не найден'} · auth: <code>{utils.escape_html(auth_state)}</code>"
            )
        if status:
            out.append(f"<code>{utils.escape_html(str(status)[:400])}</code>")
        return "\n".join(out)

    async def _install_external_provider(self, provider: str):
        meta = OMNI_EXTERNAL_PROVIDERS.get(provider)
        if not meta:
            return False, "unknown provider"
        npm_bin = shutil.which("npm") or self._get_local_npm_binary()
        if not npm_bin or (os.path.sep in npm_bin and not os.path.isfile(npm_bin)):
        ...
        return False, err[-220:] or f"exit={proc.returncode}"

    def _build_provider_auth_guide(self, provider: str) -> str:
        auth_state = self._detect_external_auth(provider)
        guides = {
            "codex": (
                "1) <code>codex login</code>\n"
                "2) Завершите авторизацию в браузере.\n"
                f"3) Статус: <code>{utils.escape_html(auth_state)}</code>"
            ),
            "gemini": (
                "1) <code>gemini auth login</code>\n"
                "2) Подтвердите вход через Google в браузере.\n"
                f"3) Статус: <code>{utils.escape_html(auth_state)}</code>"
            ),
            "claude": (
                "1) <code>claude</code> (или <code>claude login</code> если доступно)\n"
                "2) Подтвердите вход в браузере.\n"
                f"3) Статус: <code>{utils.escape_html(auth_state)}</code>"
            ),
        }
        body = guides.get(provider, "Нет инструкции.")
        return f"• <b>{provider}</b>\n{body}"

    def _detect_external_auth(self, provider: str) -> str:
        home = os.path.expanduser("~")
        checks = {
            "codex": [
                os.path.join(home, ".codex", "auth.json"),
                os.path.join(home, ".codex", "config.yaml"),
            ],
            "gemini": [
                os.path.join(home, ".config", "gemini", "credentials.json"),
                os.path.join(home, ".gemini", "credentials.json"),
            ],
            "claude": [
        ...
        for path in checks.get(provider, []):
            if os.path.exists(path):
                return f"detected ({os.path.basename(path)})"
        return "not detected"
